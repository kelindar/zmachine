...
func New(buffer []uint8, header ZHeader) *ZMachine {
	return &ZMachine{
		Buf:    buffer,
		Header: header,
		IP:     uint32(header.IP),
...
func Load(stream io.Reader) (*ZMachine, error) {
	var zm ZMachine
	...
	/*out, err := snappy.Decode(out, file)
	if err != nil {
		return nil, err
	}*/

	return &zm, binary.Unmarshal(out, &zm)
}

// Save stores the machine.
func (zm *ZMachine) Save() ([]byte, error) {
	buffer, err := binary.Marshal(zm)
	if err != nil {
		return nil, err
	}
	...
	//var out []byte
	//return snappy.Encode(out, buffer), nil
}

// PeekByte doesn't modify IP
...
func ZRead(zm *ZMachine, args []uint16, numArgs uint16) {
	...
	// If there's a buffer pending, flush it
	zm.flush()
	if b, err := zm.Save(); err == nil {
		ioutil.WriteFile("save.bin", b, os.ModePerm)
	}
...
func ZPrintNum(zm *ZMachine, args []uint16, numArgs uint16) {
	value := []byte(fmt.Sprintf("%d", int16(args[0])))
	zm.buffer = append(zm.buffer, value...)
}
...
func (zm *ZMachine) PrintZChar(ch uint16) {
	if ch == 13 {
		zm.buffer = append(zm.buffer, '\n')
	} else if ch >= 32 && ch <= 126 { // ASCII
		zm.buffer = append(zm.buffer, byte(ch))
	} // else ... do not bother
}
...
func (zm *ZMachine) DecodeZString(startOffset uint32) uint32 {
	...
	for i := 0; i < len(zchars); i++ {
		...
		if zc == 0 {
			...
			aindex := zc - 6
			zm.buffer = append(zm.buffer, alphabets[alphabetType][aindex])
		}
...
// If there's a buffer pending, flush it
func (zm *ZMachine) flush() {
	if len(zm.buffer) > 0 {
		fmt.Printf("%s", zm.buffer)
		zm.buffer = zm.buffer[:0]
	}
}
